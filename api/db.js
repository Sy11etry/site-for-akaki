...
function openDb() {
  ...
  `);
  const cols = db.prepare("select name from pragma_table_info('schedule')").all();
  if (!cols.some((c) => c.name === 'client_contact')) {
    db.exec('alter table schedule add column client_contact text');
  }
  return db;
}
